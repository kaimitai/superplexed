...
SDL_Surface* klib::gfx::KImage::to_sdl_surface(std::size_t p_transp_ind) const {
	int l_w = static_cast<int>(m_pixels.at(0).size());
	int l_h = static_cast<int>(m_pixels.size());
...
std::vector<SDL_Color> klib::gfx::KImage::get_palette(void) const {
	return m_palette;
}
...
klib::gfx::KImage::KImage(const std::vector<byte>& p_bytes, int p_w, int p_h, const std::vector<SDL_Color>& p_palette) : m_palette{ p_palette } {

	if (bits_per_pixel() == 2) {

		for (std::size_t i{ 0 }; i < p_bytes.size(); i += p_w / 4) {
			std::vector<byte> l_row;
			for (std::size_t j{ 0 }; j < p_w / 4; ++j) {
				byte b{ p_bytes[i + j] };
